...
class SysFileControllerTest {
    ...
    @BeforeEach
    void setUp() {
        mockMvc = MockMvcBuilders.standaloneSetup(new SysFileController(new TestSysFileService()))
    ...
    void uploadFileShouldReturnFileInfo() throws Exception {
        ...
        mockMvc.perform(multipart("/file/upload").file(file))
                ...
                .andExpect(jsonPath("$.data.downloadUrl").value("/file/download/1"));
    }
    ...
    private static class TestSysFileService implements SysFileService {
        ...
        public FileUploadVO uploadFile(MultipartFile file) {
            ...
            vo.setDownloadUrl("/file/download/1");
            return vo;
        }
